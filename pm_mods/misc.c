...
/* ========================================================== *
 * Modificacao SysMon: Chamada de Sistema (System Call)       *
 * ========================================================== */
#include "../../kernel/proc.h"

int do_get_proc_stats(void)
{
    struct proc p_kernel;
    int r;
    
    /* OBS: 'who_e' eh a variavel global do PM que diz qual o indice 
       do processo que chamou a syscall. 
       O Kernel usa esse mesmo indice para a tabela dele. */
    int slot_do_processo = who_e;

    /* A Magica:
       1. GET_PROC: Queremos dados da tabela de processos.
       2. &p_kernel: Onde salvar (tamanho JA ATUALIZADO pelo make includes).
       3. &slot_do_processo: O numero do processo (int) que queremos ler.
    */
    if ((r = sys_getinfo(GET_PROC, &p_kernel, sizeof(p_kernel), 
                         &slot_do_processo, sizeof(slot_do_processo))) != OK) {
        return r;
    }

    /* Copia os dados */
    mp->mp_reply.m2_l1 = p_kernel.p_total_runtime;
    mp->mp_reply.m2_l2 = p_kernel.p_context_switches;
