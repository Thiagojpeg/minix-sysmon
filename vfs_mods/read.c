	...
	vir_bytes buf, size_t size, endpoint_t for_e)
{
  ...
  /* >>> CODIGO DA LETRA C <<< */
  if (r == OK || r > 0) {
      size_t bytes_transferidos = (r == OK) ? cum_io : (size_t)r;
      if (rw_flag == READING) {
      rfp->fp_bytes_read += bytes_transferidos;
      } else if (rw_flag == WRITING) {
       rfp->fp_bytes_written += bytes_transferidos;
      }
}
...
if (rw_flag == READING) fp->fp_bytes_read += cum_io_incr;
  else fp->fp_bytes_written += cum_io_incr;
