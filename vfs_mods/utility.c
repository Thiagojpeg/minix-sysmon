...
/* Letra C: Busca estatisticas de I/O na tabela fproc usando o PID */
int do_get_io_stats(void) {
    pid_t target_pid = job_m_in.m_lc_vfs_readwrite.fd; 
    int i;

    /* Varre a tabela fproc procurando quem tem esse PID */
    for (i = 0; i < NR_PROCS; i++) {
        if (fproc[i].fp_pid == target_pid && fproc[i].fp_pid != 0) {
            /* Encontrou o PID! Devolve os contadores */
            job_m_out.m2_l1 = fproc[i].fp_bytes_read;
            job_m_out.m2_l2 = fproc[i].fp_bytes_written;
            return OK;
        }
    }

    /* Se rodou a tabela toda e não achou, retorna erro */
    return EINVAL;
}
