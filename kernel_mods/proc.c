...
/* SysMon: Importa relogio do sistema para calcular runtime */
...
void switch_to_user(void)
{
	 ...
	 */
	struct proc * p;
        struct proc * old_p; /* SysMon: guarda o processo antigo */
	...
	p = get_cpulocal_var(proc_ptr);
        ...
        old_p = p; /* SysMon: registra quem estava rodando */
	/*
...
not_runnable_pick_new:
	...
	if (p != old_p) {
		if (old_p != NULL) {
			...
			if (old_p->p_total_runtime_start != 0) {
				old_p->p_total_runtime += 
(get_monotonic() - old_p->p_total_runtime_start);
			}
			old_p->p_total_runtime_start = 0;
		}
		if (p != NULL) {
			p->p_total_runtime_start = get_monotonic();
		}
	}
	/* ========================================================== */
